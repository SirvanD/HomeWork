...
const planTrip = function (fromLine, fromStation, toLine, toStation) {
  ...
  const interchange = "Richmond"; // interchange value for central ref point
...
const printResult = function (stopsObj, isSameLine) {
  ...
  console.log(
    `You must travel through the following stops on the ${
      stopsObj.firstPart.line
    } line: ${stopsObj.firstPart.stops.join(", ")}.`
  );
  ...
  if (!isSameLine) {
    ...
    console.log(
      `Your journey continues through the following stops on the ${
        stopsObj.secondPart.line
      } line: ${stopsObj.secondPart.stops.join(", ")}.`
    );
